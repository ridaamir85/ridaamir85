<div align="center">

# ✦ Rida Amir ✦

### `BSCS @ PUCIT · AI/ML & Cybersecurity Enthusiast · Tech Explorer`

<br>

> *"The best way to predict the future is to build it."*

<br>

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=20&pause=1000&color=A855F7&center=true&vCenter=true&width=600&lines=Not+all+who+wander+are+lost+%E2%80%94+some+are+debugging+%F0%9F%90%9B;Exploring+AI%2C+ML+%26+Cybersecurity+%F0%9F%94%90;The+expert+in+anything+was+once+a+beginner+%E2%9C%A8)](https://git.io/typing-svg)

</div>

---

## 🌟 About Me

```cpp
Student rida = {
    .degree      = "BS Computer Science @ PUCIT - FCIT (Batch 2028)",
    .interests   = { "AI/ML", "Cybersecurity", "Tech Exploration" },
    .currently   = "Exploring AI/ML & diving into Cybersecurity",
    .role        = "Videographer OC @ FCIT Developers Club",
    .achievement = "Best Volunteer @ LUMS Project Justuju 2023",
    .motto       = "Turning curiosity into code 💫"
};
```

---

## 🛠️ Tech Stack

**Languages**

![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)

**Tools & Platforms**

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

**Currently Exploring 🌱**

![AI/ML](https://img.shields.io/badge/AI%2FML-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Cybersecurity](https://img.shields.io/badge/Cybersecurity-00C853?style=for-the-badge&logo=hackthebox&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

---

## 📂 Featured Projects

<table>
<tr>
<td width="50%">

### 🏏 [Cricket Match Simulator](https://github.com/ridaamir85/cricket-match-simulator)
Full-featured console-based cricket simulator with ball-by-ball commentary, 10 international teams, auto & manual scoring, persistent career stats.

**Tech:** `C++` `OOP` `File I/O` `Multi-file`

[![Repo](https://img.shields.io/badge/View%20Repo-181717?style=for-the-badge&logo=github)](https://github.com/ridaamir85/cricket-match-simulator)

</td>
<td width="50%">

### 🎮 [Maze Game](https://github.com/ridaamir85/maze-game)
Console-based maze game with 3 difficulty levels, real-time timer, CNIC-based player recognition, and persistent leaderboard.

**Tech:** `C++` `Windows API` `File Handling`

[![Repo](https://img.shields.io/badge/View%20Repo-181717?style=for-the-badge&logo=github)](https://github.com/ridaamir85/maze-game)

</td>
</tr>
<tr>
<td width="50%">

### 🌐 [Network Traffic Monitor](https://github.com/ridaamir85/Network-Traffic-Monitor)
Web-based tool to monitor and visualize network traffic data.

**Tech:** `HTML`

[![Repo](https://img.shields.io/badge/View%20Repo-181717?style=for-the-badge&logo=github)](https://github.com/ridaamir85/Network-Traffic-Monitor)

</td>
<td width="50%">

### 🔐 Exploring Cybersecurity
Actively learning cybersecurity concepts through certifications and hands-on practice.

**Certifications:**
`Intro to Cybersecurity` `OS Foundations`

</td>
</tr>
</table>

---

## 🏆 Highlights

| | |
|---|---|
| 🎥 | **Videographer OC** — FCIT Developers Club (FDC) |
| 📣 | **Campus Ambassador** — SOFTEC (Pakistan's largest software competition) |
| 🏅 | **Best Volunteer** — LUMS Project Justuju 2023 |
| 👩‍🏫 | **Teaching Assistant** — ICT Course, PUCIT |
| 🌐 | **500+ connections** on LinkedIn |

---

## 🤝 Let's Connect

<div align="center">

💼 **Open to internships & collaboration opportunities**

<br>

[![LinkedIn](https://img.shields.io/badge/Connect%20on%20LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rida-amir-b606a836b)
[![Email](https://img.shields.io/badge/Email%20Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ridaamircs@gmail.com)

<br>

*"Girl who codes by day, dreams of AI by night."* 🌙

</div>

<div align="center">

# `[[ Rida Amir ]] >> Loading... ✓`

### `BSCS @ PUCIT · AI/ML & Network Security Enthusiast · Aspiring Software Developer`

<br>

> *"The best way to predict the future is to build it."*

<br>

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=18&pause=1000&color=A855F7&center=true&vCenter=true&width=800&lines=Not+all+who+wander+are+lost+%E2%80%94+some+are+debugging+%F0%9F%90%9B;Exploring+AI%2C+ML+%26+Network+Security+%F0%9F%94%90;The+expert+in+anything+was+once+a+beginner+%E2%9C%A8)](https://git.io/typing-svg)

</div>

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 🌟 About Me

```cpp
Student rida = {
    .degree      = "BS Computer Science @ PUCIT - FCIT (Batch 2028)",
    .interests   = { "AI/ML", "Network Security", "Web Development" },
    .currently   = "Exploring AI/ML & building real-world projects",
    .achievement = "Best Volunteer @ LUMS Project Justuju 2023",
    .motto       = "Turning curiosity into code 💫"
};
```

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 🛠️ Tech Stack

**Languages**

![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)

**Frameworks & Tools**

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

**Currently Exploring 🌱**

![AI/ML](https://img.shields.io/badge/AI%2FML-22D3EE?style=for-the-badge&logo=tensorflow&logoColor=white)
![Network Security](https://img.shields.io/badge/Network_Security-06B6D4?style=for-the-badge&logo=hackthebox&logoColor=white)

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 📂 Featured Projects

<table>
<tr>
<td width="50%">

### 🗓️ [Weekly Habits Tracker](https://github.com/ridaamir85/weeklyhabits)
Single-page habit tracking web app with weekly grid, streak calculation, and localStorage persistence. Built with React 18 and custom hooks.

**Tech:** `React` `Tailwind CSS` `Vite` `Custom Hooks`

[![Live](https://img.shields.io/badge/Live%20Demo-22C55E?style=for-the-badge&logo=netlify&logoColor=white)](https://weeklyhabits.netlify.app)
[![Repo](https://img.shields.io/badge/View%20Repo-A855F7?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ridaamir85/weeklyhabits)

</td>
<td width="50%">

### 🌐 [Network Traffic Monitor](https://github.com/ridaamir85/Network-Traffic-Monitor)
Real-time network traffic monitoring tool that captures and analyzes live packets, identifies protocols (TCP/UDP/ICMP), and displays stats on an interactive web dashboard.

**Tech:** `Python` `Flask` `Scapy` `HTML`

[![Repo](https://img.shields.io/badge/View%20Repo-A855F7?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ridaamir85/Network-Traffic-Monitor)

</td>
</tr>
<tr>
<td width="50%">

### 🏏 [Cricket Match Simulator](https://github.com/ridaamir85/cricket-match-simulator)
Full-featured console-based cricket simulator with ball-by-ball commentary, 10 international teams, auto & manual scoring, persistent career stats across 9 modular C++ files.

**Tech:** `C++` `OOP` `Multi-file` `File I/O`

[![Repo](https://img.shields.io/badge/View%20Repo-A855F7?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ridaamir85/cricket-match-simulator)

</td>
<td width="50%">

### 🎮 [Maze Game](https://github.com/ridaamir85/maze-game)
Console-based maze game with 3 difficulty levels, real-time timer, CNIC-based player recognition, and persistent leaderboard.

**Tech:** `C++` `File Handling`

[![Repo](https://img.shields.io/badge/View%20Repo-A855F7?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ridaamir85/maze-game)

</td>
</tr>
</table>

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 🔐 Certifications

- AI for Beginners
- Introduction to Cybersecurity Awareness
- Introduction to Cybersecurity Foundations
- Operating System Foundations

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 🏆 Highlights

| | |
|---|---|
| 📣 | **Campus Ambassador** — SOFTEC (Pakistan's largest software competition) |
| 🎥 | **Videographer** — FCIT Developers Club (FDC) |
| 👩‍🏫 | **Teaching Assistant** — ICT Course, PUCIT |
| 🏅 | **Best Volunteer** — LUMS Project Justuju 2023 |
| 🏆 | **Top 15 Finalist** — Code Bees 5.0 x Queen of Iron Fist |
| 🌐 | **500+ connections** on LinkedIn |

<p align="center"><img src="https://capsule-render.vercel.app/api?type=rect&color=A855F7&height=3&width=800" /></p>

## 🤝 Let's Connect

<div align="center">

<br>

[![LinkedIn](https://img.shields.io/badge/Connect%20on%20LinkedIn-A855F7?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rida-amir-b606a836b)
[![Email](https://img.shields.io/badge/Email%20Me-7C3AED?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ridaamircs@gmail.com)

<br>

*"Girl who codes by day, dreams of AI by night."* 🌙

</div>
